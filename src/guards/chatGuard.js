 ...
 * - Agents with selectable:false or internal_only:true are blocked
 * - Agents requiring approval need admin authorization
 * - Fails closed (blocks execution) when registry is unavailable
 */
...
export async function guardChatAgent(agentId, isAdmin = false) {
  ...
  if (!registry || !registry.agents) {
    logger.error({ agentId }, "No registry found, blocking agent execution (fail-closed)");
  ...
  // Check 2: Internal-only agents blocked from chat
  ...
  // Check 3: Non-selectable agents blocked from chat
  if (agent.expose?.selectable === false) {
    logger.warn({ 
      agentId, 
      isAdmin 
    }, "Agent blocked: non-selectable agents cannot run in chat");
    throw new Error(`Agent "${agentId}" is not selectable and cannot run in chat`);
  }
...
export async function getAvailableChatAgents(isAdmin = false) {
  ...
  return registry.agents
    .filter(agent => {
      ...
      const isNotInternalOnly = agent.expose?.internal_only !== true;
      ...
      return hasChat && isNotInternalOnly && isSelectable && !isDestructive && !needsApproval && !isHighRisk;
    })
    .map(agent => ({
      id: agent.id,
      name: agent.name,
