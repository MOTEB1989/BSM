/**
 * Chat Agent Guard
 * Enforces context-based restrictions for chat endpoint
 * 
 * Rules:
 ...
 * - Agents with terminal_execution capability are blocked
...
export async function guardChatAgent(agentId, isAdmin = false) {
  ...
  // Check 2: Must be selectable
  if (agent.expose?.selectable === false) {
    logger.warn({ 
      agentId, 
      isAdmin 
    }, "Agent blocked: not selectable");
    throw new Error(`Agent "${agentId}" is not selectable for chat`);
  }

  // Check 3: Must not be internal_only
  if (agent.expose?.internal_only === true) {
    logger.warn({ 
      agentId, 
      isAdmin 
    }, "Agent blocked: internal only");
    throw new Error(`Agent "${agentId}" is internal only and cannot be used in chat`);
  }

  // Check 4: Block agents with terminal_execution capability
  ...
  if (capabilities.includes("terminal_execution")) {
    logger.warn({ 
      ...
      capabilities,
      isAdmin 
    }, "Agent blocked: terminal_execution not allowed in chat");
...
export async function getAvailableChatAgents(isAdmin = false) {
  ...
  return registry.agents
    .filter(agent => {
      ...
      const isNotInternal = agent.expose?.internal_only !== true;
      const capabilities = agent.capabilities || [];
      ...
      return hasChat && isSelectable && isNotInternal && !hasTerminalExec && !isDestructive && !needsApproval && !isHighRisk;
    })
    .map(agent => ({
      id: agent.id,
      name: agent.name,
