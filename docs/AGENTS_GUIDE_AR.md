...
|---|--------|---------|--------|
| 1 | **Raptor** (رابتور) | النائب المعيّن وقائد المستودع - صلاحية كاملة على إدارة المستودع والوكلاء والأوامر (وصول محدود - api/ci فقط) | api, ci |
...
⚠️ **تنبيه أمني:** Raptor محدود الوصول بسبب قدرات تنفيذ Terminal. الوصول متاح فقط عبر:

1. **عبر API مع مصادقة Admin**: `POST /api/agents/run` مع `agentId: "raptor-agent"` ورأس `x-admin-token`
2. **عبر Control API**: `POST /api/control/run` مع مصادقة مناسبة
3. **في سياق CI/CD**: من خلال GitHub Actions workflows

**ملاحظة:** Raptor **غير متاح** من واجهة الدردشة أو سياقات Mobile لأسباب أمنية (قدرات terminal_execution).
