...
**طريقة الاستدعاء:**
- عبر API (للاستخدام الداخلي فقط): `POST /api/agents/run` أو `POST /api/control/run` مع `agentId: "raptor-agent"`
- يجب إرسال الهيدر `x-admin-token` بقيمة `ADMIN_TOKEN` الصحيحة كما هي مهيأة في الخادم
- **ملاحظة:** نقطة النهاية `/api/agents/run` الآن محمية بواسطة `adminAuth` middleware وتطلب `x-admin-token` لجميع الطلبات

**متطلبات الأمان:**
- يتطلب سياق api أو ci وموافقة المسؤول **المسبقة**
- لا يجوز استدعاؤه مباشرة من المتصفحات أو تطبيقات الجوال أو أي عميل غير موثوق
- يجب أن يظل خلف طبقة آمنة (VPN / شبكة داخلية / بوابة إدارة محمية)
