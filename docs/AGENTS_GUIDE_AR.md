...
| صحة وكيل | `GET /api/agents/:agentId/health` | فحص صحة وكيل معيّن |

### الدردشة

| الواجهة | الرابط | الوصف |
|---------|--------|-------|
| دردشة مع وكيل | `POST /api/chat` | دردشة مع وكيل محمي بـ `guardChatAgent` (agentId: kimi-agent, legal-agent, ...) |
...
- عبر API (للاستخدام الداخلي فقط): `POST /api/agents/run` أو `POST /api/control/run` مع `agentId: "raptor-agent"`، ويجب إرسال الهيدر `x-admin-token` بقيمة `ADMIN_TOKEN` الصحيحة كما هي مهيأة في الخادم. **ملاحظة:** نقطة النهاية `/api/agents/run` الآن محمية بواسطة `adminAuth` middleware وتطلب `x-admin-token` لجميع الطلبات.
...
عند اختيار **"ذكي (تلقائي)"**، يتم استخدام موجّه نظام متخصص يحتوي على إرشادات داخلية لتحديد نوع الطلب. لاحظ أن agent-auto **لا يقوم بتوجيه تلقائي بين الوكلاء**، بل يقدم استجابة مباشرة استنادًا إلى موجّه نظام يحتوي على أمثلة لمجالات التخصص (قانوني، حوكمة، عام). التوجيه الفعلي بين الوكلاء يتطلب استدعاء واجهات API منفصلة لكل وكيل.
