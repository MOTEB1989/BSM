...
# إعدادات
MAIN_BRANCH="main"                 # اسم الفرع الرئيسي (يمكن تغييره إلى master)
AUTO_RESOLVE_STRATEGY="theirs"     # استراتيجية حل التعارضات: ours أو theirs
MAX_RETRIES=3                      # عدد محاولات إعادة المحاولة في حال فشل الدمج
RUN_TESTS=true                     # تشغيل الاختبارات بعد الدمج؟
FIX_LINT=true                      # محاولة إصلاح أخطاء التنسيق تلقائياً؟
PUSH_CHANGES=true                  # دفع التغييرات بعد النجاح؟
...
function fix_lint_issues {
    ...
    if [ -f "package.json" ]; then
        if grep -q '"lint"' package.json; then
            npm run lint -- --fix 2>/dev/null || info "فشل إصلاح lint، قد تحتاج إلى مراجعة يدوية."
        ...
        # لمشاريع Python - يمكن إضافة autopep8 أو black
...
function abort_merge {
    ...
    git reset --hard HEAD@{1} 2>/dev/null || git reset --hard ORIG_HEAD 2>/dev/null
}
...
for ((attempt=1; attempt<=MAX_RETRIES; attempt++)); do
    ...
    if [ $merge_exit_code -eq 0 ]; then
        ...
        # محاولة حل التعارضات باستخدام mergetool
        resolve_conflicts_with_mergetool
        if [ $? -eq 0 ]; then
            # إذا تم الحل، قم بإتمام عملية الدمج
            git add -A && git commit --no-edit 2>/dev/null || info "لا يوجد تغييرات جديدة بعد حل التعارضات."
            break
        else
...
if [ "$RUN_TESTS" = true ]; then
    ...
    if [ $test_result -ne 0 ]; then
        info "فشلت الاختبارات. محاولة إصلاح تلقائي إضافية؟"
        abort_merge
